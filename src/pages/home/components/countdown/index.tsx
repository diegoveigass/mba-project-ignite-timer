import { useEffect } from 'react'
import { CountdownContainer, Separator } from './styles'
import { differenceInSeconds } from 'date-fns'
import { useAppDispatch, useAppSelector } from '../../../../lib/redux/hooks'
import {
  markCurrentCycleAsFinished,
  setActiveCycle,
  setAmountSecondsPassed,
} from '../../../../lib/redux/slices/cycle-slice'

export function Countdown() {
  const amountSecondsPassed = useAppSelector(
    (state) => state.cycle.amountSecondsPassed,
  )
  const dispatch = useAppDispatch()

  const activeCycle = useAppSelector(
    state => state.cycle.activeCycle,
  )

  const totalSeconds = activeCycle
    ? activeCycle.minutesAmount * 60
    : 0

  useEffect(() => {
    let interval: ReturnType<typeof setInterval>

    if (activeCycle) {
      interval = setInterval(() => {
        const secondsDifference = differenceInSeconds(
          new Date(),
          activeCycle.startDate)

        if (secondsDifference >= totalSeconds) {
          dispatch(markCurrentCycleAsFinished())
          clearInterval(interval)
          dispatch(setActiveCycle(null))
        } else {
          dispatch(setAmountSecondsPassed(secondsDifference))
        }
      }, 1000)
    }

    return () => {
      clearInterval(interval)
    }
  }, [activeCycle, dispatch, totalSeconds])

  const currentSeconds = activeCycle
    ? totalSeconds - amountSecondsPassed
    : 0

  const minutesAmount = Math.floor(currentSeconds / 60)
  const secondsAmount = currentSeconds % 60

  const minutes = String(minutesAmount).padStart(2, '0')
  const seconds = String(secondsAmount).padStart(2, '0')

  useEffect(() => {
    if (activeCycle) {
      document.title = `${minutes}:${seconds}`
    }
  }, [minutes, seconds, activeCycle])

  return (
    <CountdownContainer>
      <span>{minutes[0]}</span>
      <span>{minutes[1]}</span>
      <Separator>:</Separator>
      <span>{seconds[0]}</span>
      <span>{seconds[1]}</span>
    </CountdownContainer>
  )
}
